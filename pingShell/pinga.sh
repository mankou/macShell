...
#脚本运行时是否输出当前时间 以方便你看日志
IS_OUTPUT_RUN_START_DATE=true

#是否输出脚本运行时长 可用于查看脚本运行多久
...
function fun_init_common {
    ...
    if [ ${IS_OUTPUT_RUN_START_DATE}X = "true"X ]
    then
        echo
        echo ======================
        echo $datestrFormat
...
# 解析选项
[ ${IS_OUTPUT_PARSE_PARAMETER}X = "true"X ] && echo 正在解析命令行选项 $*
...
echo **ok!!! result saved in follow path
